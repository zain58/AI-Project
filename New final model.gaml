...
* Name: Newfinalmodel
* Author: zainn
* Part 2 — Final Version (v5)
*
* most fixed version
* Matches Python folder structure:
* input/D1_N1000_run_001/voters.csv
* input/D1_N1000_run_001/edges.csv
*
* FIXES:
* [F1] Folder structure updated to match Python output
* [F2] Social welfare fixed: min([w1,w2]) not w1+w2
* [F3] num_agents tracked via length(voter) — data-driven
* [F4] num_agents, scenario_id, repetition_id added to CSV
...
global {
    ...
    int    repetition_id <- 1;
    int    num_agents    <- 1000;  // batch parameter — 1000/3000/5000
    ...
    action compute_global_metrics {
        ...
        // 2. Social welfare
        // [F2] FIXED: voter gets X points if ranked-X candidate goes to round 2
        // min([w1,w2]) = best rank among top2 — NOT w1+w2
    ...
    // -------------------------------------------------------
    // [F4] all metadata columns in CSV
    action save_row {
        save [
    ...
    init {
        ...
        // [F6] Distribution metadata set directly in init
        // Actual agent types come from CSV — Python already assigned them
        if scenario_id = "D1" {
            prop_stubborn  <- 0.6;
            prop_strategic <- 0.2;
            prop_mixed     <- 0.2;
        ...
        // [F1] FIXED: matches Python folder structure exactly
        // Python generates: D1_N1000_run_001/, D1_N3000_run_001/, D1_N5000_run_001/
        run_folder   <- input_folder + scenario_id + "_N" + string(num_agents) + "_run_" + run_tag + "/";
        voters_file  <- run_folder + "voters.csv";
        edges_file   <- run_folder + "edges.csv";
        results_file <- output_folder + "simulation_results_" + scenario_id + "_N" + string(num_agents) + "_run_" + run_tag + ".csv";

        write "Starting: scenario=" + scenario_id + " rep=" + repetition_id;
        write "Input:  " + run_folder;
        write "Output: " + results_file;

        // Load voters — loop based, safe
    ...
    reflex end_run when: current_day >= num_days {
        write "Completed: " + scenario_id + " N=" + num_agents + " rep=" + repetition_id;
    }
}
...
// ========================================================
// GUI — single run, [F5] until: REMOVED
// ========================================================
experiment main type: gui {
    parameter "Scenario"        var: scenario_id   among: ["D1","D2","D3","D4","D5"];
    parameter "Population size" var: num_agents    among: [1000, 3000, 5000];
...
// ========================================================
// BATCH — 150 runs = 5 scenarios x 3 sizes x 10 repetitions
// ========================================================
